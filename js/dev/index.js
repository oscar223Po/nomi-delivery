...
function panelsInit() {
  ...
  document.addEventListener("click", (e2) => {
    const openBtn = e2.target.closest("[data-open-panel]");
    const closeBtn = e2.target.closest("[data-panel-close]");
    if (openBtn) {
...
function infoInit() {
  document.addEventListener("click", function(e2) {
    if (bodyLockStatus && e2.target.closest("[data-fls-info]")) {
      bodyLockToggle();
...
function chatInit() {
  document.addEventListener("click", function(e2) {
    if (bodyLockStatus && e2.target.closest("[data-fls-chat]")) {
      bodyLockToggle();
...
class SelectConstructor {
  ...
  selectsInit(selectItems) {
    ...
    document.addEventListener("click", (function(e2) {
      this.selectsActions(e2);
    }).bind(this));
    document.addEventListener("keydown", (function(e2) {
      this.selectsActions(e2);
    }).bind(this));
    document.addEventListener("focusin", (function(e2) {
      this.selectsActions(e2);
    }).bind(this));
    document.addEventListener("focusout", (function(e2) {
      this.selectsActions(e2);
    }).bind(this));
  }
  ...
  selectInit(originalSelect, index) {
    ...
    if (originalSelect.options.length) {
      ...
      originalSelect.addEventListener("change", function(e2) {
        _this.selectChange(e2);
      });
    }
  }
  ...
  selectsActions(e2) {
    const t = e2.target, type = e2.type;
    ...
    } else if (type === "keydown" && e2.code === "Escape") {
      this.selectsСlose();
    }
  }
  ...
  selectChange(e2) {
    const originalSelect = e2.target;
    this.selectBuild(originalSelect);
    this.setSelectChange(originalSelect);
  }
...
function formInit() {
    ...
    if (forms.length) {
      for (const form of forms) {
        ...
        form.addEventListener("submit", function(e2) {
          const form2 = e2.target;
          formSubmitAction(form2, e2);
        });
        form.addEventListener("reset", function(e2) {
          const form2 = e2.target;
          formValidate.formClean(form2);
        });
      }
    }
    async function formSubmitAction(form, e2) {
  ...
  function formFieldsInit() {
    document.body.addEventListener("focusin", function(e2) {
    ...
    document.body.addEventListener("focusout", function(e2) {
      const targetElement = e2.target;
      if (targetElement.tagName === "INPUT" || targetElement.tagName === "TEXTAREA") {
        if (!targetElement.hasAttribute("data-fls-form-nofocus")) {
...
if (actions !== null) {
  actions.addEventListener("click", (e2) => {
    const button = e2.target.closest(".leader-hero__action");
    if (!button) return;
...
document.addEventListener("click", (e2) => {
  const portion = e2.target.closest("[data-fls-portion]");
  if (!portion) return;
  ...
  if (e2.target === plusBtn) {
    input.value = Number(input.value) + 1;
    body.classList.add("portion__body--active");
    return;
  }
  if (e2.target === minusBtn) {
